  ...
  <div class="ratingselect">
    <div class="rating-type border-1px">
      ...
      <span class="block" :class="{active:selectType===0}" @click="setSelectType(0)" >
        推荐<span class="count">{{positiveCount}}</span>
      </span>
      <span class="block" :class="{active:selectType===1}"  @click="setSelectType(1)">
        吐槽<span class="count">{{ratings.length-positiveCount}}</span>
      </span>
    </div>
  ...
  export default {
    ...
    methods:{
      setSelectType(selectType){
        //发布消息：触发事件时发布
        PubSub.publish('setSelectType',selectType)
      },
      toggleOnlyContent(){
        PubSub.publish('toggleOnlyContent')
      }
    }
  }
</script>
